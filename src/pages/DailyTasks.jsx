...
const DailyTasks = () => {
  ...
  return (
    ...
    >
      <h2 className="text-primary">Daily Tasks - {new Date().toLocaleDateString()}</h2>
      <div className="mb-3">
        <input
          type="text"
      ...
      <ul className="list-group">
        {tasks.map((task, index) =>
          task.completed ? <motion.li className="list-group-item" key={index}>{task.name}</motion.li> : null
        )}
      </ul>
    </motion.div>
  );
